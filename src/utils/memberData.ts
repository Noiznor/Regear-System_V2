...
const API_BASE_URL = process.env.NODE_ENV === 'production' 
  ? '' // Use relative URLs in Railway production
  : 'http://localhost:8080'; // Development server

// Always use API in production (Railway), localStorage only for local development
const USE_API = process.env.NODE_ENV === 'production' || window.location.port === '8080';
